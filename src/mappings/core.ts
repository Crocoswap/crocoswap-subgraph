...
import { updatePairDayData, updateTokenDayData, updateCrocoswapDayData, updatePairHourData } from './dayUpdates'
...
export function handleTransfer(event: Transfer): void {
  ...
  let factory = CrocoFactory.load(FACTORY_ADDRESS)
...
export function handleSync(event: Sync): void {
  ...
  let crocoswap = CrocoFactory.load(FACTORY_ADDRESS)

  // reset factory liquidity by subtracting onluy tarcked liquidity
  crocoswap.totalLiquidityETH = crocoswap.totalLiquidityETH.minus(pair.trackedReserveETH as BigDecimal)
  ...
  crocoswap.totalLiquidityETH = crocoswap.totalLiquidityETH.plus(trackedLiquidityETH)
  crocoswap.totalLiquidityUSD = crocoswap.totalLiquidityETH.times(bundle.ethPrice)
...
export function handleMint(event: Mint): void {
  ...
  crocoswap.txCount = crocoswap.txCount.plus(ONE_BI)
...
export function handleSwap(event: Swap): void {
  ...
  let crocoswap = CrocoFactory.load(FACTORY_ADDRESS)
  crocoswap.totalVolumeUSD = crocoswap.totalVolumeUSD.plus(trackedAmountUSD)
  crocoswap.totalVolumeETH = crocoswap.totalVolumeETH.plus(trackedAmountETH)
  crocoswap.untrackedVolumeUSD = crocoswap.untrackedVolumeUSD.plus(derivedAmountUSD)
  crocoswap.txCount = crocoswap.txCount.plus(ONE_BI)
  ...
  let crocoswapDayData = updateCrocoswapDayData(event)
  let token0DayData = updateTokenDayData(token0 as Token, event)
  let token1DayData = updateTokenDayData(token1 as Token, event)

  // swap specific updating
  crocoswapDayData.dailyVolumeUSD = crocoswapDayData.dailyVolumeUSD.plus(trackedAmountUSD)
  crocoswapDayData.dailyVolumeETH = crocoswapDayData.dailyVolumeETH.plus(trackedAmountETH)
  crocoswapDayData.dailyVolumeUntracked = crocoswapDayData.dailyVolumeUntracked.plus(derivedAmountUSD)
  crocoswapDayData.save()
